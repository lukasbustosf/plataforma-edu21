      ...
      case 'data_lab':
        ...
        const dataLabProps = {
          dataset: {
            ...
            columns: [
              { id: 'estudiante', name: 'Estudiante', type: 'string' as 'string', values: Array.from({ length: 20 }, (_, i) => `Estudiante ${i + 1}`) },
              { id: 'edad', name: 'Edad', type: 'number' as 'number', values: Array.from({ length: 20 }, () => Math.floor(Math.random() * 5) + 15) },
              { id: 'calificacion', name: 'Calificación', type: 'number' as 'number', values: Array.from({ length: 20 }, () => Math.floor(Math.random() * 4) + 4) },
              { id: 'materia', name: 'Materia', type: 'string' as 'string', values: Array.from({ length: 20 }, () => ['Matemáticas', 'Ciencias', 'Historia', 'Lenguaje'][Math.floor(Math.random() * 4)]) }
            ],
          ...
          questions: questions.map((q, index) => ({
            id: `question_${index}`,
            question: q.stem_md,
            type: 'chart-analysis' as 'chart-analysis',
            options: q.options_json || ['Opción A', 'Opción B', 'Opción C', 'Opción D'],
            correctAnswer: q.options_json?.indexOf(q.correct_answer) || 0,
      ...
      case 'timeline_builder':
        ...
        const timelineProps = {
          challenge: {
            ...
            subject: 'Historia',
            bloomLevel: 'Analizar' as 'Analizar',
            points: 200
          },
          ...commonCallbacks
        }
